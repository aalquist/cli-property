...
describe('Modify property', function () {
    ...
    it('moves property to specified group', (done) => {
        let cliTest = new CliTest();
        cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } --move ${ optionsToTest.group }`)
        .then(response => {
            if(response.error.message){
                assert(response.error.message.includes(`Unable to access user administration`));
            }else{
    ...
    describe('New Version', function () {
        ...
        it('creates a new version from latest', (done) => {
            ...
            .then((data) => {
                let cliTest = new CliTest();
                cliTest.exec(`node bin/akamaiProperty modify ${propertyName} --new`)
                .then(response => {
                    nextPropertyVersion++;
        ...
        it('creates a new version from --propver option', (done) => {
            ...
            .then((data) => {
                let cliTest = new CliTest();
                cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } --new --propver ${ propver }`)
                .then(response => {
                    nextPropertyVersion++;
    ...
    versionsToTest.forEach(function(version){
        ...
        describe(`runs options on ${ versionText }`, () => { 
            ...
            it('adds a note', function(done) {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${propertyName} ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --notes ${ optionsToTest.note }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('adds hosts', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --addhosts ${ optionsToTest.addhosts }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('deletes hosts', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --delhosts ${ optionsToTest.delhosts }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('modifies CPcode', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --cpcode ${ optionsToTest.cpcode }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('modifies rule format', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --ruleformat ${ optionsToTest.ruleFormat }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('modifies edgehostname', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --edgehostname ${ optionsToTest.edgeHostName }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('modifies sets variables', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --variables ${ optionsToTest.variables }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('modifies origin/forward', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --origin ${ optionsToTest.origin } --forward ${ optionsToTest.forward }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
            ...
            it('modifies sure route', (done) => {
                ...
                .then((data) => {
                    let cliTest = new CliTest();
                    cliTest.exec(`node bin/akamaiProperty modify ${ propertyName } ${ version === "new" ? "--new" : "" } ${ version !== "new" && version !== "latest" ? `--propver ${ version }` : "" } --sureroutemap ${ optionsToTest.sureRouteMap } --sureroutero ${ optionsToTest.sureRouteTo } --sureroutetohost ${ optionsToTest.sureRouteToHost }`)
                    .then(response => {
                        if(version === "new"){
                            nextPropertyVersion++;
                        }
